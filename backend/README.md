...
Hermes writes the words, ffmpeg cuts the video. No CapCut, no converter sites, no prompts.
...
![built with](https://img.shields.io/badge/built%20on-Hermes%20%C2%B7%20ffmpeg%20%C2%B7%20TypeScript-7c5cff)
...
|------|----------|---------|
| 🧠 **Generate** | a topic (*"why UPI beat credit cards"*) | Hermes brainstorms a viral angle, grounds it in **live facts** (Linkup), voices it (ElevenLabs), returns a **14–20s reel** |
...
They all collapse to the same core: **Hermes generates the words, ffmpeg burns them onto video.**

## How it works

```mermaid
flowchart LR
  U([chat: idea / clip / YT link]) --> H[Hermes gateway + skill]
...
sequenceDiagram
  actor User
  participant TG as Telegram (Hermes)
...
# run any mode from the terminal (no Hermes/Telegram/keys needed):
...
| Tool | Role |
|------|------|
| **Hermes** | the runtime users talk to — gateway, skill routing, content generation |
...
├─ gateway/        # Hermes / Telegram binding
...
| [SETUP.md](../docs/SETUP.md) | run it locally + manual steps (keys, Hermes, deploy) |
| [DEMO.md](../docs/DEMO.md) | 2-minute stage script + proof + Q&A |
| [TESTS.md](../docs/TESTS.md) | layered testing plan + known gaps |
| [masterplan.pdf](../docs/masterplan.pdf) | complete product and implementation plan |
